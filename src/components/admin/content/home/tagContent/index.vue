<template>
  <div>
    <bread-crumb itemName1="首页管理" itemName2="附加内容"/>
    <Card>
      <Button @click="add" type="primary" ghost size="small">添加</Button>
      <br>
      <br>
      <Table border :columns="columns" :data="tag_content" :loading="loadTableData"></Table>
    </Card>
  ...
  methods: {
    loadData() {
      this.http_request.axiosInit(
        this,
        this.request_url.home.content.multiple(2),
        response => {
          this.tag_content = response.data;
        },
    ...
    show(index) {
      this.$Modal.info({
        title: "查询详情",
        content: `
          <h3>标题:</h3>${this.tag_content[index].title}<br><br>
          <h3>内容:</h3>${this.tag_content[index].content}
        `
      });
    },
    edit(index, id) {
      (this.submitModalType = "put"),
        (this.modalData = {
          desc: "编辑内容",
          id: this.tag_content[index].id,
          title: this.tag_content[index].title,
          content: this.tag_content[index].content
        });
      this.showModal = true;
    },
    remove(id) {
